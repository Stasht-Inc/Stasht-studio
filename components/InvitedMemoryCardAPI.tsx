...
import { Invite, InvitesAPI } from "../services/invitesAPI";

interface InvitedMemoryCardAPIProps {
  invite: Invite;
...
export default function InvitedMemoryCardAPI({
  invite,
  onAccept,
  onReject,
  onCardRemoved
}: InvitedMemoryCardAPIProps) {
  ...
  useEffect(() => {
    const calculateTruncation = () => {
      const location = invite.memory_location || invite.memory?.location;
      if (locationRef.current && location) {
        ...
        const imageCount = invite.image_count || invite.memory?.image_count || 0;
        if (!imageCountContainer || imageCount === 0) {
          setIsLocationTruncated(false);
          return;
        }
  ...
  }, [invite.memory_location, invite.memory?.location, invite.image_count, invite.memory?.image_count]);
  ...
  const handleAccept = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      console.log(`Accepted invitation for memory: ${invite.memory.title}`);
    } catch (error) {
  ...
  const handleReject = async (e: React.MouseEvent) => {
    ...
    try {
      await InvitesAPI.rejectInvite(invite.id);
      ...
      console.log(`Rejected invitation for memory: ${invite.memory.title}`);
    } catch (error) {
  ...
  return (
    <div
      ...
      style={{ 
        backgroundImage: (invite.memory?.cover_image && invite.memory.cover_image !== "") ? `url('${invite.memory.cover_image}')` : 'none',
        backgroundColor: (invite.memory?.cover_image && invite.memory.cover_image !== "") ? 'transparent' : '#6C60FF'
      }}
    >
      {/* Fallback background when no image */}
      {(!invite.memory?.cover_image || invite.memory?.cover_image === "") && (
            ...
            <div className="box-border content-stretch flex flex-col gap-0.5 items-start justify-start p-0 relative shrink-0 w-[228px] z-[1]">
              ...
              <div className="box-border content-stretch flex flex-row gap-2 items-center justify-start px-2 py-1 relative shrink-0">
                <Avatar className="w-6 h-6">
                  <AvatarImage src={invite.sender_profile_image || invite.sender?.profile_image || ''} alt={invite.sender_name || invite.sender?.name} />
                  <AvatarFallback className="text-xs">
                    {(invite.sender_name || invite.sender?.name || 'U').charAt(0)?.toUpperCase()}
                  </AvatarFallback>
                </Avatar>
                ...
                <div className="flex flex-row items-center justify-center relative size-full">
                  <div className="box-border content-stretch flex flex-row items-center justify-center px-2 py-0 relative w-full">
                    <div className="basis-0 grow min-h-px min-w-px relative shrink-0 text-[#ffffff] text-[21px] text-left">
                      <p className="block font-semibold">{invite.memory_title || invite.memory?.title}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="box-border content-stretch flex flex-row gap-2.5 items-center justify-center px-2 py-0 relative rounded-2xl shrink-0">
                <div className="relative shrink-0 flex items-center gap-1 text-[#ffffff] text-[14px]">
                  ...
                  <p className="block text-[14px] whitespace-pre">
                    {invite.memory?.formatted_range || 
                     (invite.memory?.dates?.formatted_range) ||
                     (invite.memory?.start_date && invite.memory?.end_date ? 
                       `${new Date(invite.memory.start_date).toLocaleDateString()} - ${new Date(invite.memory.end_date).toLocaleDateString()}` : 
                       '') ||
                     'Date not specified'}
                  </p>
                </div>
              ...
              {/* Category Badge */}
              {(invite.memory.category || invite.memory.sub_category) && (
                <div className="box-border content-stretch flex flex-row gap-2.5 items-start justify-start px-2 py-1 relative rounded-2xl shrink-0">
                  <div className="inline-block">
                    <div 
                      className="px-3 py-1 rounded-full text-xs font-medium shadow-sm"
                      style={{
                        backgroundColor: invite.memory.category?.badge_style?.bg || '#FEF3C7',
                        color: invite.memory.category?.badge_style?.text || '#92400E'
                      }}
                    >
                      {invite.memory.sub_category?.name || invite.memory.category?.name || 'Uncategorized'}
                    </div>
                  </div>
                </div>
              )}
              ...
              {/* Location */}
              {(invite.memory_location || invite.memory?.location) && (
                <div className="box-border content-stretch flex flex-row gap-2.5 items-center justify-start px-2 py-0 relative rounded-2xl shrink-0 w-full max-w-full">
                  <div className="relative flex items-center gap-1 text-[#ffffff] text-[14px] min-w-0 w-full">
                    ...
                    <p 
                      ...
                      title={isLocationTruncated ? (invite.memory_location || invite.memory?.location) : undefined}
                      style={{ 
              ...
              {/* Collaborators count */}
              {(invite.counts?.collaborators || invite.memory?.collaborator_count || invite.collaborators?.length || 0) > 0 && (
                <div className="box-border content-stretch flex flex-row gap-2.5 items-center justify-start px-2 py-0 relative rounded-2xl shrink-0">
                  <div className="relative shrink-0 flex items-center gap-1 text-[#ffffff] text-[12px]">
                    <span className="text-white/80">
                      +{invite.counts?.collaborators || invite.memory?.collaborator_count || invite.collaborators?.length} more
                    </span>
                  </div>
                </div>
              )}
      ...
      {(invite.counts?.images || invite.memory?.counts?.images || invite.image_count || invite.memory?.image_count || 0) > 0 && (
        <div className="image-count-container absolute bottom-[22px] right-[22px] z-20">
          <div className="bg-black/60 backdrop-blur-sm border border-white/20 rounded-lg px-3 py-1 shadow-lg">
            <div className="text-white text-[12px] font-medium whitespace-nowrap">
              {invite.counts?.images || invite.memory?.counts?.images || invite.image_count || invite.memory?.image_count} image{(invite.counts?.images || invite.memory?.counts?.images || invite.image_count || invite.memory?.image_count) !== 1 ? 's' : ''}
            </div>
          </div>
        </div>
      )}
